...
let _effect_sql_sqlite_wasm_SqliteClient = require("@effect/sql-sqlite-wasm/SqliteClient");

//#region src/effect-sqlite-wasm/driver.ts
var EffectSQLiteWasmDatabase = class extends __sqlite_core_effect_db_ts.SQLiteEffectDatabase {
	static [__entity_ts.entityKind] = "EffectSQLiteWasmDatabase";
};
/**
...
* ```
*/
const make = effect_Effect.fn("SQLiteWasmDrizzle.make")(function* (config = {}) {
	const client = yield* _effect_sql_sqlite_wasm_SqliteClient.SqliteClient;
	...
	const db = new EffectSQLiteWasmDatabase(dialect, new require_effect_sqlite_wasm_session.EffectSQLiteWasmSession(client, dialect, relations, {
		logger,
		cache
	}), relations);
	db.$client = client;
...
exports.EffectSQLiteWasmDatabase = EffectSQLiteWasmDatabase;
exports.make = make;
