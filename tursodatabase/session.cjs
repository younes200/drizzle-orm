...
var TursoDatabaseSession = class TursoDatabaseSession extends __sqlite_core_async_session_ts.SQLiteAsyncSession {
	...
	async transaction(transaction, _config) {
		const session = new TursoDatabaseSession(this.client, this.dialect, this.relations, this.options);
		const tx = new TursoDatabaseTransaction("async", this.dialect, session, this.relations);
		return await this.client.transaction(async () => await transaction(tx))();
	}
};
var TursoDatabaseTransaction = class TursoDatabaseTransaction extends __sqlite_core_async_session_ts.SQLiteAsyncTransaction {
	static [__entity_ts.entityKind] = "TursoDatabaseTransaction";
	async transaction(transaction) {
		const savepointName = `sp${this.nestedIndex}`;
		const tx = new TursoDatabaseTransaction("async", this.dialect, this.session, this._.relations, this.nestedIndex + 1);
