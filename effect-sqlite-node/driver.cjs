...
let _effect_sql_sqlite_node_SqliteClient = require("@effect/sql-sqlite-node/SqliteClient");

//#region src/effect-sqlite-node/driver.ts
var EffectSQLiteNodeDatabase = class extends __sqlite_core_effect_db_ts.SQLiteEffectDatabase {
	static [__entity_ts.entityKind] = "EffectSQLiteNodeDatabase";
};
/**
...
* ```
*/
const make = effect_Effect.fn("SQLiteNodeDrizzle.make")(function* (config = {}) {
	const client = yield* _effect_sql_sqlite_node_SqliteClient.SqliteClient;
	...
	const db = new EffectSQLiteNodeDatabase(dialect, new require_effect_sqlite_node_session.EffectSQLiteNodeSession(client, dialect, relations, {
		logger,
		cache
	}), relations);
	db.$client = client;
...
exports.EffectSQLiteNodeDatabase = EffectSQLiteNodeDatabase;
exports.make = make;
