		...
		if ((0, __entity_ts.is)(column, __column_ts.Column)) {
			...
			switch (column.columnType) {
				...
				case "SQLiteBlobBuffer":
					...
					return __sql_sql_ts.sql`hex(${name}) as ${__sql_sql_ts.sql.identifier(key)}`;
				case "SQLiteNumeric":
				...
				case "SQLiteCustomColumn":
					if (!inJson) return __sql_sql_ts.sql`${name} as ${__sql_sql_ts.sql.identifier(key)}`;
					return __sql_sql_ts.sql`${column.jsonSelectIdentifier(name, __sql_sql_ts.sql)} as ${__sql_sql_ts.sql.identifier(key)}`;
