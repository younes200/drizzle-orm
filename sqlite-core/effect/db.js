...
var SQLiteEffectDatabase = class {
	static [entityKind] = "SQLiteEffectDatabase";
	query;
	constructor(dialect, session, relations, forbidJsonb) {
		...
		this.query = {};
		for (const [tableName, relation] of Object.entries(relations)) this.query[tableName] = new RelationalQueryBuilder(void 0, relations, relations[relation.name].table, relation, dialect, session, forbidJsonb, SQLiteEffectRelationalQuery);
	...
	*/
	update(table) {
		return new SQLiteUpdateBuilder(table, this.session, this.dialect, void 0, SQLiteEffectUpdateBase);
	}
	/**
	* Creates an insert query.
	*
	...
	*/
	insert(into) {
		return new SQLiteInsertBuilder(into, this.session, this.dialect, void 0, SQLiteEffectInsertBase);
	}
	/**
	* Creates a delete query.
	*
	...
	*/
	...
	run(query) {
		...
		return new SQLiteEffectRaw(this.session.prepareQuery(builtQuery, "raw", false, "run"), sequel, builtQuery);
	}
	all(query) {
		const sequel = typeof query === "string" ? sql.raw(query) : query.getSQL();
		const builtQuery = this.dialect.sqlToQuery(sequel);
		return new SQLiteEffectRaw(this.session.prepareQuery(builtQuery, "objects", false, "all"), sequel, builtQuery);
	}
	get(query) {
		const sequel = typeof query === "string" ? sql.raw(query) : query.getSQL();
		const builtQuery = this.dialect.sqlToQuery(sequel);
		return new SQLiteEffectRaw(this.session.prepareQuery(builtQuery, "objects", false, "get"), sequel, builtQuery);
	}
	values(query) {
		const sequel = typeof query === "string" ? sql.raw(query) : query.getSQL();
		const builtQuery = this.dialect.sqlToQuery(sequel);
		return new SQLiteEffectRaw(this.session.prepareQuery(builtQuery, "objects", false, "values"), sequel, builtQuery);
	}
...
//#endregion
export { SQLiteEffectDatabase, withReplicas };
