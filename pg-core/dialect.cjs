...
var PgDialect = class {
	...
	*/
	buildSelection(fields, { isSingleTable = false, ignoreCastCodecs = false } = {}) {
		...
		const chunks = fields.flatMap(({ field }, i) => {
			...
			if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) && field.isSelectionField) {
				const column = ignoreCastCodecs ? void 0 : (0, __utils_ts.getColumnFromDecoder)(field);
				...
				const column = ignoreCastCodecs ? void 0 : (0, __utils_ts.getColumnFromDecoder)(query);
				if (isSingleTable) {
					const newSql = new __sql_sql_ts.SQL(query.queryChunks.map((c) => {
						if ((0, __entity_ts.is)(c, __pg_core_columns_index_ts.PgColumn)) return __sql_sql_ts.sql.identifier(c.name);
						return c;
					}));
			...
			} else if ((0, __entity_ts.is)(field, __column_ts.Column)) {
				let name;
				...
				chunk.push(field.isAlias ? __sql_sql_ts.sql`${casted} as ${field}` : casted);
				...
				if (entries.length === 1) {
					...
					let fieldDecoder;
					if ((0, __entity_ts.is)(entry, __column_ts.Column)) {
						column = entry;
						fieldDecoder = entry;
					} else if ((0, __entity_ts.is)(entry, __sql_sql_ts.SQL)) {
						column = !ignoreCastCodecs ? (0, __utils_ts.getColumnFromDecoder)(entry) : void 0;
				...
				if (column && !field._.isWith) {
					const innerCasted = this.codecs.apply(column, "cast", __sql_sql_ts.sql`(${field._.sql})`);
					chunk.push(__sql_sql_ts.sql`${innerCasted} ${__sql_sql_ts.sql.identifier(field._.alias)}`);
				} else chunk.push(column ? this.codecs.apply(column, "cast", field) : field);
			}
	...
	buildSelectQuery({ withList, fields, fieldsFlat, where, having, table, joins, orderBy, groupBy, limit, offset, lockingClause, distinct, setOperators, comment, ignoreSelectionCastCodecs }) {
		const fieldsList = fieldsFlat ?? (0, __utils_ts.orderSelectedFields)(fields, void 0, this.codecs);
	...
	buildRqbColumn(table, field, key, inJson) {
		if ((0, __entity_ts.is)(field, __column_ts.Column)) {
			...
			return __sql_sql_ts.sql`${inJson && field.jsonSelectIdentifier ? field.jsonSelectIdentifier(name, __sql_sql_ts.sql, field.dimensions) : this.codecs.apply(field, inJson ? "castInJson" : "cast", name)} as ${__sql_sql_ts.sql.identifier(key)}`;
		}
		if ((0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased)) {
			const column = (0, __utils_ts.getColumnFromDecoder)(field);
			const q = __sql_sql_ts.sql`${table}.${__sql_sql_ts.sql.identifier(field.fieldAlias)}`;
			return __sql_sql_ts.sql`${column ? this.codecs.apply(column, inJson ? "castInJson" : "cast", q) : q} as ${__sql_sql_ts.sql.identifier(key)}`;
		}
		if ((0, __sql_index_ts.isSQLWrapper)(field)) {
			const column = (0, __utils_ts.getColumnFromDecoder)(field);
	...
	resolveSelection(field, key, inJson) {
		if ((0, __entity_ts.is)(field, __column_ts.Column)) return {
			key,
			field,
			codec: this.codecs.get(field, inJson ? "normalizeInJson" : "normalize"),
			arrayDimensions: field.dimensions
		};
		const decoderColumn = (0, __utils_ts.getColumnFromDecoder)(field);
