...
var SQLJsSession = class extends SQLiteSession {
	...
	prepareQuery(query, fields, executeMethod) {
		return new PreparedQuery(this.client, query, this.logger, fields, executeMethod, this.options.useJitMappers);
	}
	prepareRelationalQuery(query, fields, executeMethod, customResultMapper, config) {
		return new PreparedQuery(this.client, query, this.logger, fields, executeMethod, this.options.useJitMappers, customResultMapper, true, config);
	}
...
var PreparedQuery = class extends SQLitePreparedQuery {
	...
	allRqbV2(placeholderValues) {
		...
		while (stmt.step()) rows.push(stmt.getAsObject());
		stmt.free();
		return this.useJitMappers ? (this.jitMapper = this.jitMapper ?? makeJitRqbMapper(this.rqbConfig))(rows) : customResultMapper(rows, normalizeFieldValue);
	}
	...
	getRqbV2(placeholderValues) {
		...
		return this.useJitMappers ? (this.jitMapper = this.jitMapper ?? makeJitRqbMapper(this.rqbConfig))([row]) : customResultMapper([row], normalizeFieldValue);
	}
